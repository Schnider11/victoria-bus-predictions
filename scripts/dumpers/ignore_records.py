...
def get_new_file_name(date: datetime.datetime, rec_type):
    file_name = ""
    if rec_type == "trips":
...
def ignore_records(path, rec_type):
    ...
    output_file = open(path + "/" + get_new_file_name(date, rec_type), "w")
    ...
    for f in [output_file, orig_file]:
        f.close()
...
def main():
    ...
    if "trip" in args[1]:
        ignore_records(args[0], "trips")
    elif "vehicle" in args[1]:
        ignore_records(args[0], "vehicle")
